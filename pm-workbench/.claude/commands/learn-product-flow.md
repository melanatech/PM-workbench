...
2. **Compare against the paper trail — dispatch to `internal-docs-reader`** for Confluence docs, Jira tickets, and any SharePoint/file-browser design or architecture docs for this flow, so that reading happens off to the side while you keep walking the live product. The gaps and contradictions between actual behavior and documentation are the primary finding — list each one.
3. **Map to data:** which QuickSight metrics and FullStory segments cover this flow; which steps are dark (no instrumentation).
