...
- `discovery-source-reader` (`/discovery`) — Slack channels + support pages + FullStory
- `competitive-capture-agent` (`/competitive-scan`) — several competitor sites at once
- `return-window-scanner` (`/return-brief`) — weeks of Slack/Jira/Confluence/SharePoint history; the single heaviest read in the kit
